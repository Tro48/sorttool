...
const logSection = main.querySelector(
	config.dot + config.pageClasses.sectionLog
);
const logTemplate = page.querySelector(
	config.dot + config.templatesClasses.logMessage
).content;
const tabTemplate = page.querySelector(
	config.dot + config.templatesClasses.tabItems
).content;
const settingsTemplate = page.querySelector(
	config.dot + config.templatesClasses.contentSettingsItem
).content;
const tagTemplate = page.querySelector(
	config.dot + config.templatesClasses.tagItem
).content;
const tabsSection = main.querySelector(
	config.dot + config.pageClasses.tabItemsBlock
);
const settingsItemsSection = main.querySelector(
	config.dot + config.pageClasses.settingsContent
);
const buttonAddRootFolder = main.querySelector(
	config.dot + config.pageClasses.addFolderButton
);
const buttonPlay = main.querySelector(
	config.dot + config.pageClasses.buttonPlay
);
const buttonSettings = main.querySelector(
	config.dot + config.pageClasses.settingsActiveButton
);
...
const initialApp = () => {
	...
	Promise.all([getSettingsScript, getSettingsApp]).then(
		([settingsScript, settingsApp]) => {
...
buttonAddRootFolder.addEventListener('click', () => {
	...
	addFolder.then((res) => {
		...
		tabsSection
			.querySelector(`#n${res}`)
			.classList.add(config.activeClasses.tabActive);
	});
});
...
function renderLogMessage({ message, error }, messageColor) {
	...
	if (lastBlockNotification) {
		if (
			!lastBlockNotification.classList.contains('message-notification') &
			(message === config.dot)
		) {
			const logItemBlock = logTemplate
				.querySelector(config.dot + config.pageClasses.logItemBlock)
				.cloneNode(true);
			const logMessage = logItemBlock.querySelector(
				config.dot + config.pageClasses.logMessageItem
			);
			...
			lastBlockNotification.lastElementChild.textContent =
				lastBlockNotification.textContent + message;
		} else {
...
function renderSettings(settings) {
	...
	if (settingsId.length) {
		...
		settingsId.forEach((id) => {
			...
			const tabItem = tabTemplate
				.querySelector(config.dot + config.pageClasses.tabItem)
				.cloneNode(true);
			const settingsItem = settingsTemplate
				.querySelector(config.dot + config.pageClasses.tabItemContent)
				.cloneNode(true);
			const tabItemText = tabItem.querySelector(
				config.dot + config.pageClasses.tabItemText
			);
			const inputNameFolder = tabItem.querySelector(
				config.dot + config.pageClasses.inputNameFolder
			);
			const rootFolderAdress = settingsItem.querySelector(
				config.dot + config.pageClasses.rootFolderAdress
			);
			const defaultFolderAdress = settingsItem.querySelector(
				config.dot + config.pageClasses.defaultFolderAdress
			);
			const updateRootFolderUrl = settingsItem.querySelector(
				config.dot + config.pageClasses.updateRootFolderUrl
			);
			const updateDefaultFolderUrl = settingsItem.querySelector(
				config.dot + config.pageClasses.updateDefaultFolderUrl
			);
			const newTagForm = settingsItem.querySelector(
				config.dot + config.pageClasses.formTagAdd
			);
			const addTagFolderPathButton = settingsItem.querySelector(
				config.dot + config.pageClasses.addTagFolderPathButton
			);
			const buttonDeleteFolder = tabItem.querySelector(
				'.button-delete-folder-settings'
			);
			...
			updateRootFolderUrl.addEventListener('click', (evt) => {
				renderFolderElement(
					evt,
					config.pageClasses.tabItemContent,
					config.pageClasses.rootFolderAdress,
					'folderPath'
				);
			});
			updateDefaultFolderUrl.addEventListener('click', (evt) => {
				renderFolderElement(
					evt,
					config.pageClasses.tabItemContent,
					config.pageClasses.defaultFolderAdress,
					'dirDefault'
				);
			});
			addTagFolderPathButton.addEventListener('click', (evt) => {
				...
				newFolderDir.then((newDir) => {
					evt.target.closest(
						config.dot + config.pageClasses.inputTagLable
					).children[0].value = newDir;
				});
			});
			newTagForm.addEventListener('submit', (evt) => {
				evt.preventDefault();
				const submitData = Array.from(evt.target.elements)
			...
			tabItemText.addEventListener('dblclick', () => {
				editFolderName(tabItemText, inputNameFolder);
			});
...
function editFolderName(tabItemText, inputNameFolder) {
	...
	main.addEventListener(
		'click',
		(evt) => {
			saveFolderName(evt, tabItemText, inputNameFolder, abortController);
		},
		{ signal: abortController.signal }
	);
	inputNameFolder.addEventListener(
		'keydown',
		(evt) => {
			saveFolderName(evt, tabItemText, inputNameFolder, abortController);
		},
	...
	if (evt.key === 'Enter' || !document.querySelector(':focus')) {
		getSettings.then((settings) => {
			...
			setSettings.then((res) => {
				...
				inputNameFolder.classList.remove(
					config.activeClasses.tabInputNameActive
				);
...
function renderTagItem(settings, rootFolderId) {
	...
	const tagSection = settingsContent.querySelector(
		config.dot + config.pageClasses.tagContainer
	);
	tagSection.textContent = '';

	Object.keys(settings[rootFolderId].dirList).forEach((tagFolder) => {
		const tagItem = tagTemplate
			.querySelector(config.dot + config.pageClasses.tagItem)
			.cloneNode(true);
		tagItem.querySelector(
			config.dot + config.pageClasses.tagItemTagContent
		).textContent = tagFolder;
		tagItem.querySelector(
			config.dot + config.pageClasses.tagItemDirContent
		).textContent = settings[rootFolderId].dirList[tagFolder];
		tagItem.setAttribute('id', tagFolder);
		tagItem
			.querySelector(config.dot + config.pageClasses.button)
			.addEventListener('click', deleteTagItem);
...
function deleteTagItem(evt) {
	const getSettings = window.preload.getSettings(settingsFile, {});
	const itemElement = evt.target.closest(
		config.dot + config.pageClasses.tagItem
	);
	const rootFolderId = itemElement.closest(
		config.dot + config.pageClasses.tabItemContent
	).id;
	let itemId = itemElement.id;
	getSettings.then((settings) => {
		delete settings[rootFolderId].dirList[itemId];
		if (Object.keys(settings[rootFolderId].dirList).length) {
			const newDirListTags = Object.keys(settings[rootFolderId].dirList)
				.join('_')
				.split('_');
...
function pushTab(evt) {
	const tabsList = tabsSection.querySelectorAll(
		config.dot + config.pageClasses.tabItem
	);
